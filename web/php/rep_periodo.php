...
<body>    
    ...
    <div class="col">
        <form method="POST" action="rep_periodo.php">
            <p>Periodos:
                <select name="periodo" required>
                    <option value="0">Seleccione:</option>
                    <?php
                    $query2 ="SELECT * FROM periodos";
                    $result2=mysqli_query($con, $query2);

                    while ($valores2 = mysqli_fetch_array($result2)) {
                        echo '<option value="'.$valores2['id_periodo'].'">'.$valores2['descrip'].'</option>';
                    }
                    ?>
                </select>
            </p>
        ...
        <?php
            if(isset($_POST['periodo'])){
                ...
                $sql="SELECT usuarios.user As Alumno, nota1,nota2,nota3 FROM notas
                        ...
                        WHERE notas.id_periodo= $periodo;";
                include('listar.php');
